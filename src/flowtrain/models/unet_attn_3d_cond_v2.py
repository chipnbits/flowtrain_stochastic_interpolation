...
class MixATb(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv3d(self.dim_in * 2, self.dim_in, 3, padding=1)
        self.act = nn.SiLU()
        self.conv2 = nn.Conv3d(self.dim_in, self.dim_in, 3, padding=1)

    def forward(self, x, ATb):
        mixed = torch.cat((x, ATb), dim=1)
        mixed = self.conv1(mixed)
        mixed = self.act(mixed)
        mixed = self.conv2(mixed)

        return mixed


class EmbedMixATb(nn.Module):
    """
    Single block that:
      1) Interpolates and embeds ATb,
      2) Concatenates it with x,
      3) Learns how to fuse them together.

    Input:
        x   : (B, C_x, H*scale, W*scale, D*scale)
        ATb : (B, C_0, H, W, D)
    Output:
        (B, C_x, H, W, D)  # or (B, C_out, H, W, D)
    """
    ...
    def __init__(self, dim_in, dim_out=None, scale_factor=1.0):
        ...
        self.scale_factor = scale_factor
        self.dim_in = dim_in
        self.dim_out = dim_out if dim_out is not None else self.dim_in

        # ATb embedding to reach scale
        self.embed_conv1 = nn.Conv3d(
            self.dim_in, self.dim_out, kernel_size=5, padding=2
        )
        self.embed_act = nn.SiLU()
        self.embed_conv2 = nn.Conv3d(
            self.dim_out, self.dim_out, kernel_size=5, padding=2
        )

        # Mix ATb with x
        self.mix_conv1 = nn.Conv3d(
            self.dim_out + self.dim_out, self.dim_out, 3, padding=1
        )
        self.mix_act = nn.SiLU()
        self.mix_conv2 = nn.Conv3d(self.dim_out,self.dim_out, 3, padding=1)

    def forward(self, x, ATb):
        # Interpolate to scale of x
        if self.scale_factor != 1.0:
            ATb = F.interpolate(
                ATb,
        ...
        # Embed ATb to match channel size of x
        ATb = self.embed_conv1(ATb)
        ATb = self.embed_act(ATb)
        ATb = self.embed_conv2(ATb)

        # Mix embedded ATb with x
        residual = x
        mixed = torch.cat((x, ATb), dim=1)  # channels = dim_in + dim_out
        mixed = self.mix_conv1(mixed)
        mixed = self.mix_act(mixed)
        mixed = self.mix_conv2(mixed)
...
class Unet3DCond(Module):
    ...
    def __init__(
        ...
        for ind, (
            ...
            self.downs.append(
                ModuleList(
                    [
                        EmbedMixATb(self.data_channels, dim_in, scale_factor=scale),
        ...
        for ind, (
            ...
            self.ups.append(
                ModuleList(
                    [
                        EmbedMixATb(self.data_channels, dim_out, scale_factor=scale),
    ...
    def forward(self, x, ATb, time, x_self_cond=None):
        ...
        for ATb_embed_mix, block1, block2, attn, downsample in self.downs:

            x = ATb_embed_mix(x, ATb_opened)
        ...
        for ATb_embed_mix, block1, block2, attn, upsample in self.ups:
            x = ATb_embed_mix(x, ATb_opened)
