...
class EmbedATb(nn.Module):
    ...
    def __init__(self, dim_in, dim_out=None, scale_factor=1.0):
        ...
        self.conv1 = nn.Conv3d(self.dim_in, self.dim_out, 3, padding=1)
        self.act = nn.SiLU()
        self.conv2 = nn.Conv3d(self.dim_out, self.dim_out, 3, padding=1)
...
class ResnetBlock(Module):
    ...
    def __init__(self, dim, dim_out, *, time_emb_dim=None, A_emb_dim=None, dropout=0.0):
        super().__init__()
        self.time_mlp = (
        ...
        self.A_mlp = (
            nn.Sequential(nn.Conv3d(A_emb_dim, dim_out, kernel_size=5, padding=2), nn.SiLU())
            if exists(A_emb_dim)
            else None
        )
    ...
    def forward(self, x, time_emb=None, A_emb=None):
        ...
        if exists(self.A_mlp) and exists(A_emb):
            A_emb = self.A_mlp(A_emb)
            x = x + A_emb  # Add directly into the feature map before `block2`
...
class Unet3DCond(Module):
    ...
    def __init__(
        ...
        self.init_conv_ATb = nn.Conv3d(self.data_channels, self.data_channels, 7, padding=3)
    ...
    def forward(self, x, ATb, time, x_self_cond=None):
        ...
        assert(x.shape == ATb.shape), f"Input and ATb shapes do not match: {x.shape} and {ATb.shape}"
        ...
        for ATb_embed, block1, block2, attn, downsample in self.downs:
            
            ATb_embedded = ATb_embed(ATb_opened)
            x = x + ATb_embedded
